...
bool TileCheck::ContinueCheckingAllRoomTemplates()
{
	...
	if ( m_iCurrentRoomTemplate >= CLevelTheme::s_pCurrentTheme->m_RoomTemplates.Count() )
		return false;
	
	CRoomTemplate *pTemplate = CLevelTheme::s_pCurrentTheme->m_RoomTemplates[m_iCurrentRoomTemplate];
	if ( !pTemplate )
	{
		m_iCurrentRoomTemplate++;
		m_iCheckedTemplates++;
