...
bool VMFExporter::ProcessSideKey( KeyValues *pKey )
{
	...
	{
		if ( !strnicmp( szValue, "tools/", 6 ) )
			m_bMaterialIsDisplacementOrTool = true;
		else if ( strstr( szValue, "blend" ) || strstr( szValue, "BLEND" ) )
			m_bMaterialIsDisplacementOrTool = true;
		else
			m_bMaterialIsDisplacementOrTool = false;
	}
	...
	{
		if ( m_bMaterialIsDisplacementOrTool )
			return true;
...
ConVar tilegen_use_tilegen_instances( "tilegen_use_tilegen_instances", "1", FCVAR_REPLICATED | FCVAR_CHEAT );

bool VMFExporter::ProcessEntity( KeyValues *pEntityKeys )
{
	if ( tilegen_use_tilegen_instances.GetBool() && pEntityKeys->GetString( "classname" ) &&
		!Q_stricmp( pEntityKeys->GetString( "classname" ), "tilegen_instance" ) )
	{
		if ( !pEntityKeys->GetString( "tag" ) )
			return false;

		if ( pEntityKeys->GetInt( "chance" ) && pEntityKeys->GetInt( "chance" ) <= rand() % 100 )
		{
			pEntityKeys->SetString( "classname", "info_null" );
		...
		Q_snprintf( wildcard, sizeof(wildcard), "tilegen/instances/%s/%s/*.vmf", 
			m_pRoom->m_pRoomTemplate->m_pLevelTheme->m_szName,
			pEntityKeys->GetString( "tag" ) );
 
		const char *filenames[256];
		const char *pFileName = g_pFullFileSystem->FindFirstEx( wildcard, "GAME", &findHandle );
		while ( pFileName && nFiles < 255 )
		{
			filenames[nFiles++] = pFileName;
		...
		Q_snprintf( filename, sizeof(wildcard), "../tilegen/instances/%s/%s/%s", 
			m_pRoom->m_pRoomTemplate->m_pLevelTheme->m_szName,
			pEntityKeys->GetString( "tag" ), filenames[rand() % nFiles] );

		pEntityKeys->SetString( "classname", "func_instance" );
		pEntityKeys->SetString( "file", filename );

		return ProcessEntity( pEntityKeys );
	}
...
bool VMFExporter::ProcessEntityKey( KeyValues *pKey )
{
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		if ( !stricmp(szKey, pUnique) && szValue && szValue[0] != '@' && szValue[0] != '#' && stricmp( szValue, "structure_seal" ) ) // don't make names unique if they start with special character
		{
...
bool VMFExporter::ProcessConnectionsKey( KeyValues *pKey )
{
	...
	if ( szValue && szValue[0] != '@' && szValue[0] != '#' && szValue[0] != '!' )		// don't make names unique if they start with special character
	{
		Q_snprintf( buffer, sizeof( buffer ), "Room%d_%s", m_iCurrentRoom, szValue );		
	}
	else
...
bool VMFExporter::AddLevelContainer()
{
	...
	KeyValues *pStructureSeal = new KeyValues( "entity" );
	pStructureSeal->SetInt( "id", ++m_iEntityCount );
	pStructureSeal->SetString( "classname", "func_brush" );
	pStructureSeal->SetString( "targetname", "structure_seal" );
	for ( KeyValues *pKeys = pWorldKeys->GetFirstSubKey(); pKeys; pKeys = pKeys->GetNextKey() )
	{
		const char *szName = pKeys->GetName();
		if ( !Q_stricmp( szName, "solid" ) )
		{
			pStructureSeal->AddSubKey( pKeys->MakeCopy() );
		}
	}
	m_pExportKeys->AddSubKey( pStructureSeal );
