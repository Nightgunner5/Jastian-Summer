...
bool CAlienSwarm::SpawnMarineAt( CASW_Marine_Resource * RESTRICT pMR, const Vector &vecPos, const QAngle &angFacing, bool bResurrection )
{
	...
	// create the marine
	CASW_Marine * RESTRICT pMarine;
	bool bPreExisting;
	if (pMR->GetProfile() && gEntList.FindEntityByNameFast( NULL, AllocPooledString(pMR->GetProfile()->m_ShortName) ) )
	{
		pMarine = (CASW_Marine*) gEntList.FindEntityByNameFast( NULL, AllocPooledString(pMR->GetProfile()->m_ShortName) );
		bPreExisting = true;
	}
	else
	{
		pMarine = (CASW_Marine*) CreateEntityByName( "asw_marine" );
		bPreExisting = false;
	}
...
#if 0 // Get rid of debrief in Jastian Summer
...
void CAlienSwarm::AlienKilled(CBaseEntity *pAlien, const CTakeDamageInfo &info)
{
	...
	if ( pAlien && !Q_strcmp( "asw_shieldbug", pAlien->GetClassname() ) )
		pSB = dynamic_cast<CASW_Shieldbug*>(pAlien);
...
{
	return false; // Jastian Summer: TEST, trying to get AutoSave working.
