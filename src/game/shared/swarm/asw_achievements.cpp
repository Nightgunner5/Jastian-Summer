...
void CASW_Achievement::IncrementCount()
{
	if ( !GetClientModeASW()->IsOfficialMap() )
	{
		if ( cc_achievement_debug.GetBool() )
			Msg( "Achievement %s not incremented - not on official map.", GetName() );
		return;
	}
...
const char *g_szAchievementMapNames[] = {
	"jas01_tutorial01",
	"jas02_maintainencetunnel"
};

// =====================

bool g_bHasMovedForward;
class CAchievement_Backward_Thinker : public CASW_Achievement
{
	void Init()
	{
	...
	virtual void ListenForEvents( void )
	{
		...
		g_bHasMovedForward = false;
	}

	void FireGameEvent_Internal( IGameEvent *event )
	{
		if ( !Q_stricmp( event->GetName(), "mission_success" ) && ASWGameRules() && !g_bHasMovedForward )
		{
...
DECLARE_ACHIEVEMENT_ORDER( CAchievement_Backward_Thinker, ACHIEVEMENT_JAS_BACKWARD_THINKER, "JAS_BACKWARD_THINKER", 5, 10 );

// =====================

class CAchievement_Grub_Squash : public CASW_Achievement
{
	void Init()
	{
...
DECLARE_ACHIEVEMENT_ORDER( CAchievement_Grub_Squash, ACHIEVEMENT_JAS_GRUB_SQUASH, "JAS_GRUB_SQUASH", 5, 20 );

// =====================

class CAchievement_Flashlight_Hero : public CASW_Achievement
{
	void Init()
	{
	...
	bool m_bWasPlayerHurt;

	virtual void ListenForEvents( void )
	{
		ListenForGameEvent( "mission_success" );
		ListenForGameEvent( "marine_hurt" );
		m_bWasPlayerHurt = false;
	}
	...
	void FireGameEvent_Internal( IGameEvent *event )
	{
		if ( !Q_stricmp( event->GetName(), "mission_success" ) && ASWGameRules() && !g_bHasMovedForward )
		{
			...
			const char *szMapName = event->GetString( "strMapName" );
			if ( !Q_stricmp( szMapName, "jas02_maintainencetunnel" ) )
				IncrementCount();
		}
		else if ( !Q_stricmp( event->GetName(), "marine_hurt" ) )
		{
			m_bWasPlayerHurt = true;
		}
	}
};
DECLARE_ACHIEVEMENT_ORDER( CAchievement_Flashlight_Hero, ACHIEVEMENT_JAS_FLASHLIGHT_HERO, "JAS_FLASHLIGHT_HERO", 5, 30 );

// =====================
#if 0 // These achievements are from Alien Swarm, not Jastian Summer.
