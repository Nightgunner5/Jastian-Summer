...
BEGIN_NETWORK_TABLE( CASW_Holdout_Mode, DT_ASW_Holdout_Mode )
	...
	RecvPropArray3( RECVINFO_ARRAY( m_nCurrentScore ), RecvPropInt( RECVINFO( m_nCurrentScore[0] ) ) ),
...
//	RecvPropTime( RECVINFO( m_flResupplyEndTime ) ),
	...
	SendPropArray3( SENDINFO_ARRAY3( m_nCurrentScore ), SendPropInt( SENDINFO_ARRAY( m_nCurrentScore ) ) ),
		...
		case HOLDOUT_STATE_SHOWING_WAVE_SCORE:
			{
				...
				{
					if ( m_nUnlockedResupply > 0 || m_Waves[ GetCurrentWave() ]->WaveHasResupply() )
					{
...
void CASW_Holdout_Mode::OnAlienKilled( CBaseEntity *pAlien, const CTakeDamageInfo &info )
{
	...
	if ( pAlien->Classify() == CLASS_ASW_BUZZER ) 
	{
		CASW_Buzzer *pBuzzer = static_cast<CASW_Buzzer*>( pAlien );
		if ( !pBuzzer || !pBuzzer->IsHoldoutAlien() )
			return;
	}
	else if ( IsAlienClass( pAlien->Classify() ) )
	{
		CASW_Alien *pAlienNPC = static_cast<CASW_Alien*>( pAlien );
		if ( !pAlienNPC || !pAlienNPC->IsHoldoutAlien() )
			return;
	}
	else
	{
	...
	CASW_Marine *pMarine = CASW_Marine::AsMarine( info.GetAttacker() );
	if ( pMarine )
	{
		...
		int iResourceIndex = pGameResource->GetMarineResourceIndex( pMR );
		if ( iResourceIndex != -1 )
		{
			m_nCurrentScore.Set( iResourceIndex, m_nCurrentScore.Get( iResourceIndex ) + iAlienValue );
		}
	}
}
...
void CASW_Holdout_Mode::ResurrectDeadMarines()
{
	...
	{
		CASW_Marine_Resource *pMR = ASWGameResource()->GetMarineResource( i );
		if ( pMR && pMR->GetHealthPercent() <= 0 && m_nMarineLastRespawned[i] < gpGlobals->curtime - 1.0f ) // if marine exists, is dead, and hasn't respawned in the last second
		{
			CBaseEntity *pMarineRagdoll = gEntList.FindEntityByName( NULL, pMR->GetProfile()->m_ShortName );
			if (pMarineRagdoll) {
				UTIL_Remove( pMarineRagdoll );
				ASWGameRules()->Resurrect( pMR, pAliveMarine );
			}
			m_nMarineLastRespawned[i] = gpGlobals->curtime;
...
int CASW_Holdout_Mode::GetCurrentScore()
{
#ifdef CLIENT_DLL
	...
	if ( pMarine )
	{
		...
		if ( pMR )
		{
			...
			if (pGameResource)
			{
				int iResourceNumber = pGameResource->GetMarineResourceIndex( pMR );
				if ( iResourceNumber != -1 )
				{
					return m_nCurrentScore.Get( iResourceNumber );
				}
			}
		}
	}
	...
	int score = 0;
	for ( int i = 0; i < m_nCurrentScore.Count(); i++ ) {
		score += m_nCurrentScore.Get( i );
	}
	return score;
}

int CASW_Holdout_Mode::GetCurrentScore( int i )
{
	return m_nCurrentScore.Get( i );
}
