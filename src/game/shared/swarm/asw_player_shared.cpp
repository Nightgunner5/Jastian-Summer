...
#ifdef CLIENT_DLL
				C_JaS_Alien_Controller *pAlienController = C_JaS_Alien_Controller::AutoList().Count() ? dynamic_cast<C_JaS_Alien_Controller *>( C_JaS_Alien_Controller::AutoList().Head() ) : NULL;
#else
				CJaS_Alien_Controller *pAlienController = dynamic_cast<CJaS_Alien_Controller *>( gEntList.FindEntityByClassname( NULL, "jas_alien_controller" ) );
#endif

				if ( pAlienController && pAlienController->m_hAlien )
				{
#ifdef GAME_DLL
					CASW_Alien *pAlien = pAlienController->m_hAlien.Get();
					Vector vecMoveLocal = Vector();

					if ( ucmd->buttons & IN_FORWARD && !( ucmd->buttons & ( IN_BACK ) ) )
						vecMoveLocal.x = 100;
					else if ( ucmd->buttons & IN_BACK && !( ucmd->buttons & ( IN_FORWARD ) ) )
						vecMoveLocal.x = -100;
					
					if ( ucmd->buttons & IN_MOVERIGHT && !( ucmd->buttons & ( IN_MOVELEFT ) ) )
						vecMoveLocal.y = -100;
					else if ( ucmd->buttons & IN_MOVELEFT && !( ucmd->buttons & ( IN_MOVERIGHT ) ) )
						vecMoveLocal.y = 100;

					vecMoveLocal.z = 10; // Sometimes, the movement code is buggy and makes the alien try to move through the floor.

					Vector &vecMoveTo = Vector();
					pAlien->EntityToWorldSpace( vecMoveLocal, &vecMoveTo );

					DebugDrawLine( pAlien->GetAbsOrigin(), vecMoveTo, 255, 0, 0, true, 1.0f );

					//Ray_t ray;
					//ray.Init( pAlien->GetAbsOrigin(), vecMoveTo );
					trace_t tr;
					UTIL_TraceHull( pAlien->GetAbsOrigin(), vecMoveTo, pAlien->GetHullMins(), pAlien->GetHullMaxs(),
						MASK_NPCSOLID, pAlien, COLLISION_GROUP_NPC, &tr );
					//enginetrace->TraceRay( ray, MASK_NPCSOLID, new CTraceFilterWorldAndPropsOnly(), &tr );
					DebugDrawLine( pAlien->GetAbsOrigin(), tr.endpos, 255, 255, 0, true, 1.0f );

					SetAbsOrigin( vecMoveTo );

					/*if ( ucmd->buttons & ( IN_ATTACK | IN_ATTACK2 | IN_ALT1 ) )
					{
						CAI_ASW_MeleeBehavior *pMelee;
						if ( pAlien->GetBehavior<CAI_ASW_MeleeBehavior>( &pMelee ) )
						{
							pMelee->SetSchedule( 0, pMelee->SelectSchedule() );
							pAlien->SetSchedule( pMelee->GetCurSchedule( 0 ) );
							return;
						}
					}*/

					if ( ucmd->buttons & IN_JUMP && pAlien->Classify() == CLASS_ASW_DRONE )
					{
						CASW_Drone_Advanced *pDrone = dynamic_cast<CASW_Drone_Advanced *>( pAlien );
						pDrone->InputEnableJump( inputdata_t() );

						Vector vecRayEnd;
						pDrone->EntityToWorldSpace( Vector( 1000, 0, 50 ), &vecRayEnd );
						Ray_t ray;
						ray.Init( pDrone->EyePosition(), vecRayEnd );
						trace_t tr;
						enginetrace->TraceRay( ray, MASK_NPCSOLID, new CTraceFilterWorldAndPropsOnly(), &tr );
						
						Vector vecJumpVelocity;
						vecJumpVelocity += tr.endpos;
						vecJumpVelocity -= pDrone->GetAbsOrigin();
						pDrone->DoForcedJump( vecJumpVelocity );
						ucmd->buttons &= ~IN_JUMP;
						return;
					}
					pAlien->SetAlienOrders( AOT_MoveToIgnoringMarines, vecMoveTo, this );
#endif
					return;
				}

				if ( pAlienController && pAlienController->m_hBuzzerOverride )
				{
#ifdef GAME_DLL
					CASW_Buzzer *pBuzzer = pAlienController->m_hBuzzerOverride.Get();
					if ( UTIL_EntityInSolid( pBuzzer ) )
					{
						Vector vecUnstuckPos;
						if ( pBuzzer->FindNearestValidGoalPos( pBuzzer->GetAbsOrigin(), &vecUnstuckPos ) )
							pBuzzer->SetAbsOrigin( vecUnstuckPos );
						else
							DevMsg( pBuzzer, "Could not unstick buzzer" );
					}
					...
					float fMoveLocalZ = AngleNormalize( ( ucmd->viewangles - pBuzzer->GetAbsAngles() )[PITCH] );

					vecMoveLocal.z = fMoveLocalZ > 30 ? 250 : ( fMoveLocalZ < -30 ? -250 : 0 );

					pBuzzer->SetAbsAngles( ucmd->viewangles );

					Vector &vecMoveTo = Vector();
					pBuzzer->EntityToWorldSpace( vecMoveLocal, &vecMoveTo );

					trace_t tr;
					UTIL_TraceHull( pBuzzer->GetAbsOrigin(), vecMoveTo, pBuzzer->GetHullMins(), pBuzzer->GetHullMaxs(),
						MASK_NPCSOLID, pBuzzer, COLLISION_GROUP_NPC, &tr );

					SetAbsOrigin( tr.endpos );
					pBuzzer->SetAlienOrders( AOT_MoveToIgnoringMarines, tr.endpos, this );
					pBuzzer->MoveToTarget( 0.1f, tr.endpos );
#endif
					return;
				}

				CBaseEntity *pCharacter = m_hMarine.Get();
				pCharacter->SetMoveType( MOVETYPE_WALK );
				MarineMove()->SetupMarineMove( this, pCharacter, ucmd, moveHelper, g_pMoveData);
				g_pMarineGameMovement->ProcessMovement(this, pCharacter, g_pMoveData);
				MarineMove()->FinishMarineMove( this, pCharacter, ucmd, g_pMoveData );
...
void CASW_Player::ItemPostFrame()
{
	...
	if ( pAlienController && ( pAlienController->m_hAlien.Get() || pAlienController->m_hBuzzerOverride.Get() ) )
	{
		if ( pWeapon )
			pWeapon->ItemBusyFrame();
	}
	else if ( gpGlobals->curtime < next_attack )
	{
		if ( pWeapon )
		{
			pWeapon->ItemBusyFrame();
...
Vector CASW_Player::EyePosition( )
{
	...
	if ( pAlienController && pAlienController->m_hAlien )
		return pAlienController->m_hAlien->EyePosition();
	if ( pAlienController && pAlienController->m_hBuzzerOverride )
		return pAlienController->m_hBuzzerOverride->EyePosition();
...
const QAngle& CASW_Player::EyeAngles( )
{
	...
	if ( pAlienController && pAlienController->m_hAlien )
	{
		return pAlienController->m_hAlien->EyeAngles();
	}
	if ( pAlienController && pAlienController->m_hBuzzerOverride )
	{
		return pAlienController->m_hBuzzerOverride->EyeAngles();
	}

	if (GetMarine())
	{
		if ( angAdjustedEyes.x > 60 )
			angAdjustedEyes.x = 60;
		angAdjustedEyes.z = 0;
