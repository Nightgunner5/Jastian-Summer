...
bool CASW_MarineGameMovement::CheckJumpButton( void )
{
	...
	trace_t ceiling;
	TraceMarineBBox( marine->GetAbsOrigin(), marine->GetAbsOrigin() + Vector( mv->m_vecVelocity.x * ASW_JUMP_LATERAL_SCALE,
		mv->m_vecVelocity.y * ASW_JUMP_LATERAL_SCALE, sqrt(2 * asw_marine_gravity.GetFloat() * GAMEMOVEMENT_ASW_JUMP_HEIGHT) ) / 5,
		MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, ceiling );
	if ( ceiling.DidHit() )
		return false;
	...
	// fixme: should play from the marine, not the player
	Vector vecSrc = mv->GetAbsOrigin();
	player->PlayStepSound( vecSrc, marine->m_pSurfaceData, 0.75f, true );
