...
//-----------------------------------------------------------------------------
class CPointCommentaryNode : public CBaseAnimating, public IASW_Server_Usable_Entity
{
...
public:
	...
	virtual void ActivateUseIcon( CASW_Marine* pMarine, int nHoldType ) { if ( nHoldType == 0 ) PlayerActivated(); }
...
public:
	...
	CPointCommentaryNode *GetNodeUnderCrosshair()
	{
		...
		trace_t tr;
		Vector vecSrc = pMarine->EyePosition();
		Vector vecForward = pMarine->Forward();

		g_bTracingVsCommentaryNodes = true;
		UTIL_TraceLine( vecSrc, vecSrc + vecForward * MAX_TRACE_LENGTH, MASK_SOLID, pMarine, COLLISION_GROUP_NONE, &tr );
