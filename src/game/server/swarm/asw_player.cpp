...
void CASW_Player::PostThink()
{
	...
	if ( IsHLTV() && ASWGameResource() )
	{
		ASWGameResource()->m_bPlayerReady.Set(entindex() - 1, true);
...
int CASW_Player::OnTakeDamage( const CTakeDamageInfo &info )
{
	if ( GetMarine() )
		GetMarine()->TakeDamage( info ); // Make "hurt me" cheats actually hurt the player.
