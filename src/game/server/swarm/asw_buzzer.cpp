...
void CASW_Buzzer::ClampMotorForces( Vector &linear, AngularImpulse &angular )
{
	...
	angular.x *= scale;
	angular.y *= scale;
	angular.z = clamp( angular.z * scale, GetASWSenses()->IsBeingControlled() ? -300 : -1000, GetASWSenses()->IsBeingControlled() ? 300 : 1000 );
...
const QAngle &CASW_Buzzer::EyeAngles()
{
	Vector eyeposition;
...
Vector CASW_Buzzer::EyePosition()
{
