...
{
	if ( GlobalEntity_GetState( "jas_playercontrolledmarineview" ) == GLOBAL_ON && ASWGameResource()->GetMarineResource( 0 ) )
	{
		CASW_Marine *pMarine = dynamic_cast<CASW_Marine *>( GetOuter() );
		CASW_Marine *pPlayer = ASWGameResource()->GetMarineResource( 0 )->GetMarineEntity();
		// = dynamic_cast<CASW_Player *>( UTIL_GetLocalPlayer() )->GetMarine();
		
		if ( pPlayer && pMarine && !Q_stricmp( pMarine->GetEntityNameAsCStr(), "jack" ) )
			return pPlayer->IsInFieldOfView( dest ) || pPlayer->GetAbsOrigin().DistTo( dest ) < 64.0f;
	}
	if ( GlobalEntity_GetState( "jas_playercontrolledmarineview" ) == GLOBAL_DEAD && CASW_Marine::AsMarine( GetOuter() ) &&
		!Q_stricmp( CASW_Marine::AsMarine( GetOuter() )->GetEntityNameAsCStr(), "jack" ) )
		return false;
...
void CASW_AI_Senses::SetBeingControlled( bool bBeingControlled )
{
	m_bBeingControlled = bBeingControlled;
}

bool CASW_AI_Senses::ShouldSeeEntity( CBaseEntity *pEntity )
{
	return !m_bBeingControlled && BaseClass::ShouldSeeEntity( pEntity );
}

bool CASW_AI_Senses::CanSeeEntity( CBaseEntity *pSightEnt )
{
	return !m_bBeingControlled && BaseClass::CanSeeEntity( pSightEnt );
}
