...
BEGIN_DATADESC( CASW_Simple_Alien )
	...
	DEFINE_KEYFIELD( m_bAllowEnemies, FIELD_BOOLEAN, "allow_enemies" ),
END_DATADESC()
...
void CASW_Simple_Alien::FindNewEnemy()
{
	...
	if (CanSee(pNearest) && m_bAllowEnemies)
	{
		SetEnemy(pNearest);
	}
}
...
void CASW_Simple_Alien::WhatToDoNext(float delta)
{
	if (!m_bAllowEnemies)
	{
		Vector vecMoveDestination = PickRandomDestination( 128, vec3_origin );
		if ( !enginetrace->PointOutsideWorld( vecMoveDestination ) )
			SetMoveTarget( vecMoveDestination );
		SetNextThink( gpGlobals->curtime + 0.1f );
	}
	else if (m_iState == ASW_SIMPLE_ALIEN_ATTACKING)
	{
