...
#define JAS_COLONIST_NAMETAG_MODEL "models/props_jastiansummer/colonistidentifier.mdl"
...
CASW_Marine::~CASW_Marine()
{
	...
	if ( m_PlayerColonistBack )
		UTIL_Remove( m_PlayerColonistBack );
...
#if 0 // Don't do this - we're in firstperson now.
...
#else
	// Make the player's bounding box slightly bigger to avoid "head through wall" bug
	Vector vecSurroundingMins( -20, -20, 0 );
	Vector vecSurroundingMaxs( 20, 20, 72 );
	CollisionProp()->SetCollisionBounds( vecSurroundingMins, vecSurroundingMaxs );
#endif
...
void CASW_Marine::Precache()
{
	...
	PrecacheModel( "models/jas_colonist/colonist.mdl" );
	PrecacheModel( JAS_COLONIST_NAMETAG_MODEL );
}
...
{
	if ( m_pProfileOverride )
		return m_pProfileOverride;
...
bool CASW_Marine::KeyValue( const char *szKeyName, const char *szValue )
{
	...
	if( !bResult )
	{
		...
		else if ( !Q_stricmp( szKeyName, "starting_primary" ) )
		{
			CBaseCombatWeapon *pWeapon = dynamic_cast<CBaseCombatWeapon *>( CreateEntityByName( szValue ) );
			if ( pWeapon )
			{
				pWeapon->Spawn();
		...
		else if ( !Q_stricmp( szKeyName, "starting_secondary" ) )
		{
			CBaseCombatWeapon *pWeapon = dynamic_cast<CBaseCombatWeapon *>( CreateEntityByName( szValue ) );
			if ( pWeapon )
			{
				...
				pWeapon->m_iClip1 = 9999;
				GiveAmmo(9999, pWeapon->GetPrimaryAmmoType());
				Weapon_Equip_In_Index( pWeapon, 1 );
			}
			bResult = true;
		}
		else if ( !Q_stricmp( szKeyName, "constant_slow_heal" ) )
		{
			m_bConstantSlowHeal = true;
			m_fNextSlowHealTick = gpGlobals->curtime + ASW_MARINE_HEALTICK_RATE;
			bResult = true;
		}
		else if ( !Q_stricmp( szKeyName, "squad_id" ) )
		{
			if ( g_SquadFormations.IsValidIndex( g_SquadFormations.Find( szValue ) ) )
			{
				m_hSquadFormation = g_SquadFormations.Element( g_SquadFormations.Find( szValue ) );
				m_hSquadFormation->Add( this );
			}
			else
			{
				m_hSquadFormation = static_cast<CASW_SquadFormation *>( CreateEntityByName( "asw_squadformation" ) );
				m_hSquadFormation->Leader( this );
				g_SquadFormations.Insert( szValue, m_hSquadFormation.Get() );
			}
			bResult = true;
		}
	}
...
void CASW_Marine::InhabitedBy(CASW_Player *player)
{
	...
	if ( GlobalEntity_GetState( "jas_firstperson" ) )
	{
		SetModelName( MAKE_STRING( "models/jas_colonist/colonist.mdl" ) );
		SetModel( "models/jas_colonist/colonist.mdl" );
		m_PlayerAnimState->InitASW( this, this, LEGANIM_8WAY, false );
		m_nSkin = (int) GlobalEntity_GetState( "jas_player_colonist_type" );

		// Reset all the pose parameters
		SetPoseParameter( "move_yaw", 0.0f );
		SetPoseParameter( "body_yaw", 0.0f );
		SetPoseParameter( "spine_yaw", 0.0f );
		SetPoseParameter( "neck_trans", 0.0f );
		...
		SetPoseParameter( "gesture_height", 0.0f );
		SetPoseParameter( "gesture_width", 0.0f );

		if ( !m_PlayerColonistBack )
		{
			m_PlayerColonistBack = dynamic_cast<CDynamicProp *>( CreateEntityByName( "prop_dynamic" ) );
			m_PlayerColonistBack->SetModelName( MAKE_STRING( JAS_COLONIST_NAMETAG_MODEL ) );
			m_PlayerColonistBack->SetModel( JAS_COLONIST_NAMETAG_MODEL );
			m_PlayerColonistBack->m_nSkin = 9;
			m_PlayerColonistBack->SetParent( this );
			m_PlayerColonistBack->SetParentAttachment( "SetParentAttachment", "back", false );
			m_PlayerColonistBack->Spawn();
		}
	}
...
void CASW_Marine::UninhabitedBy(CASW_Player *player)
{
	...
	if ( GlobalEntity_GetState( "jas_firstperson" ) )
	{
		m_PlayerAnimState->InitASW( this, this, LEGANIM_9WAY, false );
	}
...
int CASW_Marine::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( newInfo.GetDamage() > 0 )
	{
		...
		if ( IsInhabited() )
		{
			...
			if (player)
			{
				gameeventmanager->FireEvent( gameeventmanager->CreateEvent( "marine_hurt" ) );
...
void CASW_Marine::PostThink()
{
	...
	// Colonist melee animations don't contain the needed event, so we have to fake it.
	if ( IsInhabited() )
	{
		// There's only one inhabited "marine", so this is OK
		static bool bDidMeleeDamage = false;
		if ( GetCurrentMeleeAttack() && !Q_strnicmp( "MeleeAttack", GetCurrentMeleeAttack()->m_szSequenceName, 11 ) )
		{
			float fCycle = 0;
			int iSequence = LookupSequence( GetCurrentMeleeAttack()->m_szSequenceName );
			for ( int iLayer = 0; iLayer <= MAX_OVERLAYS; iLayer++ )
			{
				if ( GetLayerSequence( iLayer ) == iSequence )
				{
					fCycle = GetLayerCycle( iLayer );
					break;
				}
			}
			if ( !bDidMeleeDamage && fCycle >= 0.5f )
			{
				DoMeleeDamageTrace( 0, 0 );
	...
	if ( IsInhabited() )
	{
		const QAngle eyes = GetCommander()->EyeAngles();
		SetPoseParameter( "head_yaw", AngleNormalize( eyes[YAW] - GetAbsAngles()[YAW] ) );
		SetPoseParameter( "head_pitch", AngleNormalize( eyes[PITCH] - GetAbsAngles()[PITCH] ) );
		SetPoseParameter( "aim_yaw", AngleNormalize( eyes[YAW] - GetAbsAngles()[YAW] ) );
		SetPoseParameter( "aim_pitch", AngleNormalize( eyes[PITCH] - GetAbsAngles()[PITCH] ) );
		for ( int i = 0; i < ASW_MAX_EQUIP_SLOTS; i++ )
		{
			if ( GetWeapon( i ) && dynamic_cast<CJaS_Weapon_Flashlight *>( GetWeapon( i ) ) )
			{
				dynamic_cast<CJaS_Weapon_Flashlight *>( GetWeapon( i ) )->OrientFlashlight( eyes );
			}
		}
	}
}
...
void CASW_Marine::ASWThinkEffects()
{
	...
	if ( (m_bSlowHeal || IsInfested() || m_bConstantSlowHeal || (IsInhabited() && GetHealth() < GetMaxHealth())) && GetHealth() > 0 )
	{
		while (gpGlobals->curtime >= m_fNextSlowHealTick)
		{
			...
			if (m_bConstantSlowHeal && GetHealth() < GetMaxHealth())
			{
				SetHealth( GetHealth() + 1 );
			}
			else if (IsInhabited() && GetHealth() < GetMaxHealth() && GetLastDamageTime() < m_fNextSlowHealTick - 5)
			{
				// Regenerate player health after they've been out of combat for at least five seconds.
				SetHealth( GetHealth() + 1 );
			}
...
{
	return IsInhabited() ? m_flGroundSpeed : m_fCachedIdealSpeed;	
}
...
{
	CASW_Marine_Resource *pMR = dynamic_cast<CASW_Marine_Resource*>(m_MarineResource.Get());

	// Prevent crashing when using NPC marines
	if ( m_pProfileOverride && pMR )
	{
		pMR->SetProfileIndex( m_pProfileOverride->m_ProfileIndex );
	}
...
bool CASW_Marine::BecomeRagdollOnClient( const Vector &force )
{
	...
	if (asw_marine_server_ragdoll.GetBool() || ASWHoldoutMode())
	{
...
CASW_SquadFormation *CASW_Marine::GetSquadFormation() // get the formation in which I participate; in the future there may be more than one per universe
{
	if ( !m_hSquadFormation.Get() )
	{
		m_hSquadFormation = static_cast<CASW_SquadFormation *>( CreateEntityByName( "asw_squadformation" ) );
		m_hSquadFormation->Leader( this );
	}
