...
const char *pszAntlionGibs_Unique[NUM_ANTLION_GIBS_UNIQUE] = {
	"models/aliens/drone/gib_torso.mdl"
};

#define	NUM_ANTLION_GIBS_MEDIUM	4
...
#define	NUM_ANTLION_GIBS_SMALL	1
...
#ifdef JASTIANSUMMER_DLL
		CASW_Marine *pPlayer = CASW_Marine::AsMarine( pHurt );
...
Vector VecAntlionCheckThrowTolerance( CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed, float flTolerance )
{
...
bool CNPC_Antlion::GetSpitVector( const Vector &vecStartPos, const Vector &vecTarget, Vector *vecOut )
{
	...
	Vector vecToss = VecAntlionCheckThrowTolerance( this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat(), (10.0f*12.0f) );

	// If this failed then try a little faster (flattens the arc)
	if ( vecToss == vec3_origin )
	{
		vecToss = VecAntlionCheckThrowTolerance( this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat() * 1.5f, (10.0f*12.0f) );
		if ( vecToss == vec3_origin )
			return false;
	}
