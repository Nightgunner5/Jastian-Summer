...
BEGIN_DATADESC( CASW_Simple_Drone )
	DEFINE_KEYFIELD( m_bIsHologram, FIELD_BOOLEAN, "is_hologram" ),
END_DATADESC()
...
{
	ResetSequence(LookupSequence("attack01"));
	m_flPlaybackRate = 1.25f;
}
...
int CASW_Simple_Drone::OnTakeDamage( const CTakeDamageInfo &info )
{
	if ( m_bIsHologram )
	{
		ResetSequence( LookupSequence( "BindPos" ) );
		SetThink( &CBaseEntity::SUB_StartFadeOutInstant );
