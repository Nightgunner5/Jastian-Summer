...
void CASW_Alien::CallBehaviorThink()
{
	...
	if ( pCurrent /*&& !GetASWSenses()->IsBeingControlled()*/ )
	{
...
{
	if ( GetASWSenses()->IsBeingControlled() )
		SetEnemy( NULL );
...
void CASW_Alien::ClearAlienOrders()
{
	...
	m_vecAlienOrderSpot = vec3_origin;
	m_AlienOrderObject = NULL;
	m_bIgnoreMarines = GetASWSenses()->IsBeingControlled();
	m_bFailedMoveTo = false;
}
...
const QAngle &CASW_Alien::EyeAngles()
{
	Vector eyeposition;
	static QAngle eyeangles;

	if ( GetAttachment( "eyes", eyeposition, eyeangles ) )
	{
		return eyeangles;
	}
...
{
	return !GetASWSenses()->IsBeingControlled() && BaseClass::IsValidEnemy( pEnemy );
}
