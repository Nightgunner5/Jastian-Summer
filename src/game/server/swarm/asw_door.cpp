...
BEGIN_DATADESC(CASW_Door)
	...
	DEFINE_OUTPUT( m_OnStartSeal,		"OnStartSeal" ),
	DEFINE_OUTPUT( m_OnHalfwaySealed,	"OnHalfwaySealed" ),
	DEFINE_OUTPUT( m_OnFullySealed,		"OnFullySealed" ),
	DEFINE_OUTPUT( m_OnStartCut,		"OnStartCut" ),
	DEFINE_OUTPUT( m_OnHalfwayCut,		"OnHalfwayCut" ),
...
void CASW_Door::WeldDoor( bool bSeal, float fAmount, CASW_Marine *pMarine )
{
	...
	if ( fNewTime > 0 && fCurrentSealTime <= 0 )
	{
		m_OnStartSeal.FireOutput(pMarine, this);
	}

	if ( fNewTime < GetTotalSealTime() && fCurrentSealTime >= GetTotalSealTime() )
	{
		m_OnStartCut.FireOutput(pMarine, this);
	}

	if ( fNewTime >= GetTotalSealTime() / 2 && fCurrentSealTime < GetTotalSealTime() / 2 )
	{
		m_OnHalfwaySealed.FireOutput(pMarine, this);
	}

	if ( fNewTime <= GetTotalSealTime() / 2 && fCurrentSealTime > GetTotalSealTime() / 2 )
	{
		m_OnHalfwayCut.FireOutput(pMarine, this);
	}
