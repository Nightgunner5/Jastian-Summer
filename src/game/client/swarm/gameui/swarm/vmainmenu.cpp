...
static bool g_bJustLoadedMainMenu;
	...
	BaseClass( parent, panelName, true, true, false, false )
{
	...
	{
		SetVisible( false );
		g_bJustLoadedMainMenu = true;

		char cmd[256];
		KeyValues *pChapterBackgrounds = new KeyValues( "chapters" );
		pChapterBackgrounds->LoadFromFile( filesystem, "scripts/ChapterBackgrounds.txt" );
		
		const char *mapname = mapname = pChapterBackgrounds->GetString( sv_unlockedchapters.GetString(), "1" );
		V_snprintf( cmd, sizeof( cmd ), "map_background %s", mapname );
		engine->ClientCmd_Unrestricted( cmd );
	}
	else
	{
		engine->ClientCmd_Unrestricted( "stopsound; unpause; play jastiansummer/music/greensummers" );

		if ( g_bJustLoadedMainMenu )
		{
			vgui::ImagePanel *pBackgroundFade = new vgui::ImagePanel( this, "backgroundfade" );
			int w, h;
			engine->GetScreenSize( w, h );
			char img[256];
			V_snprintf( img, sizeof( img ), "../console/%s%s", engine->GetLevelName() + 5, (float) w / h < 1.7f ? "" : "_widescreen" ); // "maps/" is always 5 characters long.
			pBackgroundFade->SetImage( img );
			pBackgroundFade->SetSize( w, h );
			pBackgroundFade->SetShouldScaleImage( true );
			pBackgroundFade->SetMouseInputEnabled( false );
			vgui::GetAnimationController()->RunAnimationCommand( pBackgroundFade, "alpha", 0, 2, 2, vgui::AnimationController::INTERPOLATOR_ACCEL );
			pBackgroundFade->SetAutoDelete( true );
			g_bJustLoadedMainMenu = false;
		}

		int day, month, year;
		GetCurrentDate( &day, &month, &year );
		ConVar *mat_reverse_depth = cvar->FindVar( "mat_reversedepth" );
		if ( day == 1 && month == 4 )
		{
			mat_reverse_depth->SetValue( 1 );
		}
		else
		{
			mat_reverse_depth->SetValue( 0 );
		}
	}
}
...
void MainMenu::OnCommand( const char *command )
{
	...
	else if( !Q_strcmp( command, "CreateGame" ) )
	{
		...
		pSettings->SetString( "game/campaign", "jastiansummer" );
		pSettings->SetString( "game/mission", "jas00_intro" );
...
void MainMenu::ApplySchemeSettings( IScheme *pScheme )
{
	...
	const char *pSettings = "Resource/UI/BaseModUI/MainMenu_JastianSummer.res";
