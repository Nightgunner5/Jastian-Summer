...
void C_ASW_Objective::UpdateMarkings( void )
{
	...
	if ( pMap )
	{
		...
		for ( int i = 0; i < IObjectiveMarkerList::AutoList().Count(); ++i )
		{
			...
			//Msg( "Objective %s think: pMarker->GetObjectiveName() = %s; pMarker->GetEntityName() = %s", GetEntityName(), pMarker->GetObjectiveName(), pMarker->GetEntityName() );
			...
			if ( V_strcmp( pMarker->GetObjectiveName(), GetEntityName() ) == 0 )
			{
				...
				//Msg( "Objective think: %s (%d, %d, %d) (%d, %d, %d) -> (%d, %d) (%d, %d)\n", GetEntityName(), (int) vUpperLeft.x, (int) vUpperLeft.y, (int) vUpperLeft.z, (int) vLowerRight.x, (int) vLowerRight.y, (int) vLowerRight.z, (int) vUpperLeft.x, (int) vUpperLeft.y, (int) v2DLowerRight.x, (int) v2DLowerRight.y );
		...
		//Msg( "Objective think: %s found %d asw_marker.\n", GetEntityName(), m_iNumMapMarks );
		if ( bMarkers )
		{
